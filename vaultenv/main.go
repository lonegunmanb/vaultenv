...
func main() {
	...
	downloadInstaller, _ := pkg.NewDownloadInstaller("https://releases.hashicorp.com/vault/{{ .Version }}/vault_{{ .Version }}_{{ .Os }}_{{ .Arch }}.zip", ctx)
	goBuildInstaller := pkg.NewGoBuildInstaller("https://github.com/hashicorp/vault.git", "vault", "", ctx)
	fallbackInstaller := pkg.NewFallbackInstaller(downloadInstaller, goBuildInstaller)
	var homeDir string
	var err error
	if homeDir = os.Getenv("VAULTENV_HOME_DIR"); homeDir != "" {
	...
	env := pkg.NewEnv(homeDir, "vaultenv", "vault", fallbackInstaller)
	...
	var rootCmd = &cobra.Command{Use: "vaultenv"}
